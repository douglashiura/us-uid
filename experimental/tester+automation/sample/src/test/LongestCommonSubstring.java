...
public class LongestCommonSubstring {
	...
	public static void main(String[] args) {
		...
		int qtdeTeste = Integer.parseInt(sc.nextLine());
		for (int i = 0; i < qtdeTeste; i++) {
			int qtdeString = Integer.parseInt(sc.nextLine());
			String[] textos = new String[qtdeString];
			for (int j = 0; j < qtdeString; j++) {
				textos[j] = sc.nextLine();
			}
			LcsSolver solver = new LcsSolver(textos);
			System.out.println( solver.getLongestCommonSubstrings(qtdeString).first().length());
		}
	}
	...
	public static class LcsSolver {
		...
		// TODO(williamfiset): support LCS with strings as int arrays for larger
		// alphabet sizes.
		...
		// Build text containing sentinels. Must have computed lowest and highest ascii
		// values beforehand.
		// All sentinels values will be in the range [0, numSentinels)
		// All text values will be in the range [numSentinels, numSentinels +
		// highestAsciiValue - lowestAsciiValue]
			...
			for (int i = 0, k = 0; i < strings.length; i++) {
				...
				if (!(0 <= text[k - 1] && text[k - 1] < numSentinels)) {
					throw new IllegalStateException(
							String.format("Unexpected character range. Was: %d, wanted between [%d, %d)", text[k - 1],
									0, numSentinels));
				}
			}
		}

		// Counts the number of suffixes of different colors between [lo, hi] and
		// determines
		// if there is enough variety for a LCS candidate.
		private boolean enoughUniqueColorsInWindow(int lo, int hi) {
			// TODO(williamfiset): Avoid initializing a new hash set to count colors every
			// method call.
		...
		private void addLcs(int lo, int hi, int windowLcp) {
			if (hi - lo + 1 < k) {
				log(String.format("lo: %d, hi: %d. Too small range. lo: %d, hi: %d, k: %d, hi - lo + 1 < k", lo, hi, lo,
						hi, k));
				return;
			}
			if (windowLcp == 0) {
